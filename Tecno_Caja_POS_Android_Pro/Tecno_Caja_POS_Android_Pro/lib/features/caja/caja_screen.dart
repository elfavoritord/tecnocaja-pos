  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: sesionAsync.when(
        data: (sesion) => sesion == null ? const _AbrirCajaForm() : _SesionAbiertaView(sesion: sesion),
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('La caja está cerrada', style: Theme.of(context).textTheme.titleLarge),
              ...
              LoadingButton(label: 'Abrir caja', isLoading: _cargando, onPressed: _abrir),
            ],
          ),
        ),
      ),
...
class _SesionAbiertaView extends ConsumerWidget {
        ...
        return ListView(
          ...
          children: [
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text('Turno abierto', style: Theme.of(context).textTheme.titleMedium),
                      ],
                    ),
                    const SizedBox(height: 4),
                    Text('Desde ${Formatters.dateTime(sesion.abiertaEn)}', style: Theme.of(context).textTheme.bodySmall),
                  ],
                ),
              ),
            ),
            ...
            if (resumen == null)
              ...
              _filaResumen(context, 'Ventas en efectivo', resumen.totalVentasEfectivo),
              _filaResumen(context, 'Ventas con tarjeta', resumen.totalVentasTarjeta),
              _filaResumen(context, 'Ventas por transferencia', resumen.totalVentasTransferencia),
              _filaResumen(context, 'Ventas a crédito', resumen.totalVentasCredito),
              _filaResumen(context, 'Entradas manuales', resumen.totalEntradas),
              _filaResumen(context, 'Salidas / gastos', -(resumen.totalSalidas + resumen.totalGastos)),
              const Divider(height: 32),
              _filaResumen(context, 'Efectivo esperado en caja', resumen.montoEsperado, destacado: true),
              const SizedBox(height: 8),
              Text('${resumen.cantidadVentas} ventas en este turno', style: Theme.of(context).textTheme.bodySmall),
              const SizedBox(height: 24),
              FilledButton.icon(
                icon: const Icon(Icons.lock_outline),
                label: const Text('Cerrar caja'),
                onPressed: () => _abrirDialogoCierre(context, ref, sesion, resumen.montoEsperado),
              ),
            ],
          ],
        );
  ...
  Widget _filaResumen(BuildContext context, String etiqueta, double monto, {bool destacado = false}) {
  ...
  Future<void> _abrirDialogoCierre(BuildContext context, WidgetRef ref, SesionCaja sesion, double montoEsperado) async {
    final controller = TextEditingController(text: montoEsperado.toStringAsFixed(2));
    ...
    final montoContado = double.tryParse(controller.text.replaceAll(',', '.')) ?? montoEsperado;
    final usuarioId = ref.read(authControllerProvider).usuario?.id;
    if (usuarioId == null) return;

    final cerrada = await ref.read(cajaRepositoryProvider).cerrar(
          sesion: sesion,
