...
class PosScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: sesionAsync.when(
        data: (sesion) => sesion == null ? const _CajaCerradaAviso() : _PosContenido(sesion: sesion),
  ...
  Future<void> _escanearYAgregar(BuildContext context, WidgetRef ref) async {
    ...
    final codigo = await mostrarEscanerCodigoBarras(context, vibrarActivo: config?.vibrarScanner ?? true);
    if (codigo == null || !context.mounted) return;

    final empresaId = ref.read(authControllerProvider).empresaId;
    if (empresaId == null) return;
    final producto = await ref.read(productoRepositoryProvider).porCodigoBarras(empresaId, codigo);
    if (producto != null) {
    ...
    final crear = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('No hay ningún producto con el código "$codigo". ¿Deseas crearlo?'),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('Cancelar')),
          FilledButton(onPressed: () => Navigator.of(context).pop(true), child: const Text('Crear producto')),
        ],
      ),
    );
    if (crear == true && context.mounted) {
      await Navigator.of(context).push<bool>(
        MaterialPageRoute(builder: (_) => ProductoFormScreen(codigoBarrasInicial: codigo)),
      );
    }
  }
}
...
class _BotonVentasSesion extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final cantidad = empresaId == null
        ? 0
        : ref.watch(ventasSuspendidasProvider(empresaId)).valueOrNull?.length ?? 0;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Abre la caja para empezar a vender', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 16),
            FilledButton(onPressed: () => context.push('/caja'), child: const Text('Ir a Caja')),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
            ...
            : GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _TarjetaProducto(
                    producto: producto,
                    onTap: () => ref.read(carritoControllerProvider.notifier).agregarProducto(producto),
                  );
                },
              );
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Stack(
          children: [
            Padding(padding: const EdgeInsets.only(bottom: 76), child: panelProductos),
            Align(alignment: Alignment.bottomCenter, child: _BarraCarritoCompacta(sesion: widget.sesion)),
          ],
        );
      },
    );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(producto.nombre, maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodyMedium),
              const SizedBox(height: 4),
              Text(
                Formatters.currency(producto.precioVenta),
                style: Theme.of(context).textTheme.titleSmall?.copyWith(color: AppColors.primaryDark, fontWeight: FontWeight.bold),
              ),
            ],
          ),
        ),
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('${carrito.cantidadUnidades.toStringAsFixed(0)} artículos', style: const TextStyle(color: Colors.white)),
              Text(
...
class _PanelCarrito extends ConsumerWidget {
            ...
            child: Row(
              ...
              children: [
                ...
                if (carrito.lineas.isNotEmpty) ...[
                  ...
                  TextButton(onPressed: notifier.limpiar, child: const Text('Vaciar')),
                ],
              ],
            ),
          ),
          ListTile(
            ...
            onTap: empresaId == null
                ...
                : () async {
                    ...
                    if (cliente != null) notifier.establecerCliente(cliente.id, cliente.nombre);
                  },
          ),
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(linea.producto.nombre, maxLines: 1, overflow: TextOverflow.ellipsis),
                                    Text(
                              ...
                              IconButton(
                                ...
                                onPressed: () => notifier.cambiarCantidad(linea.producto.id, linea.cantidad - 1),
                              ),
                              Text(linea.cantidad.toStringAsFixed(linea.cantidad.truncateToDouble() == linea.cantidad ? 0 : 2)),
                              IconButton(
            ...
            child: Column(
              children: [
                ...
                SizedBox(
                  width: double.infinity,
                  child: FilledButton(
                    onPressed: carrito.estaVacio ? null : () => mostrarCheckout(context, ref, sesion),
  ...
  Future<void> _suspender(BuildContext context, WidgetRef ref, SesionCaja sesion) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Venta suspendida.')));
    }
  }
}
