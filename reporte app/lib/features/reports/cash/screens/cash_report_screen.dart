...
const _cashQueryTimeout = Duration(seconds: 12);
...
final _cashProvider = FutureProvider.autoDispose<_CashReportData>((ref) async {
  ...
  // ref.read en vez de ref.watch: evita que el FutureProvider se reinicie cada
  // vez que el StreamProvider del perfil emite (lo que causa el freeze de loading).
  // El botón de refresco usa ref.invalidate(_cashProvider) para recargar datos.
  final profile = ref.read(currentUserProfileProvider).valueOrNull;
  final branchIds = ref.read(activeBranchIdsProvider);
    ...
    return Scaffold(
      ...
      body: cashAsync.when(
        ...
        error: (error, _) => AppErrorState(
          ...
          onRetry: () => ref.invalidate(_cashProvider),
        ),
        data: (data) {
          final content = (data.registers.isEmpty && data.closings.isEmpty)
              ? const AppEmptyState(
              ...
              : _CashReportBody(data: data);

          return Column(
            children: [
              _SyncBanner(loadedAt: data.loadedAt),
...
class _CashClosingTileState extends State<_CashClosingTile> {
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveRegisterName =
        rawName.isEmpty ||
            RegExp(r'^Caja\s+\d+$', caseSensitive: false).hasMatch(rawName)
        ? 'Caja'
        : rawName;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        // Durante el layout de AnimatedCrossFade, maxWidth puede ser 0 o negativo
        // al medir ambas ramas simultáneamente. Clamp garantiza que SizedBox
        // nunca reciba un ancho negativo, lo que causaría un crash de constraints.
        final maxW = constraints.maxWidth.clamp(0.0, double.infinity);
        final isWide = maxW >= 520;
        final rawWidth = isWide ? (maxW - 36) / 4 : (maxW - 12) / 2;
        final width = rawWidth.clamp(0.0, double.infinity);
...
/// Banner que muestra fecha de consulta e indicador de sincronización.
class _SyncBanner extends StatelessWidget {
  final DateTime loadedAt;

  const _SyncBanner({required this.loadedAt});
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Sincronizado · ${DateHelpers.date(loadedAt)} · Actualizado $timeLabel',
              style: const TextStyle(
...
Future<List<CashClosingModel>> _loadCashClosings({
  ...
  try {
    // Sin orderBy para no requerir índice compuesto; ordenamos client-side.
    ...
    // permission-denied: usuario sin acceso al nivel del negocio → intentar por sucursal.
    // failed-precondition: índice faltante → misma estrategia de fallback.
