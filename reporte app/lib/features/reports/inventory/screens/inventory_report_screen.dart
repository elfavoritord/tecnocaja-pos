  ...
  Widget build(BuildContext context) {
    ...
    return ReportScaffold(
      ...
      actions: [
        if (canManage)
          IconButton(
            ...
            onPressed: () => context.push(AppRoutes.productManage),
      ...
      body: productsAsync.when(
        ...
        data: (products) => _buildContent(products, canManage: canManage),
      ),
    );
  }

  Widget _buildContent(List<ProductModel> products, {required bool canManage}) {
    // Métricas
    ...
    if (_syncFilter != _SyncFilter.all) {
      filtered = filtered.where((p) {
        if (p.origin != 'app_reporte') return false;
        switch (_syncFilter) {
          case _SyncFilter.createdFromApp:
            return true;
          case _SyncFilter.pending:
            return !p.synced && p.syncStatus != 'error';
          case _SyncFilter.synced:
            return p.synced;
          case _SyncFilter.error:
            return p.syncStatus == 'error';
                      ...
                      children: [
                        ...
                        Expanded(
                          child: _ValueChip(
                            ...
                            profit: FormatHelpers.currencyCompact(
                              potentialProfit,
                            ),
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: [
                _SyncFilterChip(
                  label: 'Todos',
                  selected: _syncFilter == _SyncFilter.all,
                  onTap: () => setState(() => _syncFilter = _SyncFilter.all),
                ),
                _SyncFilterChip(
                  label: 'Creados app',
                  selected: _syncFilter == _SyncFilter.createdFromApp,
                  onTap: () =>
                      setState(() => _syncFilter = _SyncFilter.createdFromApp),
                ),
                _SyncFilterChip(
                  label: 'Pendientes',
                  selected: _syncFilter == _SyncFilter.pending,
                  onTap: () =>
                      setState(() => _syncFilter = _SyncFilter.pending),
                ),
                _SyncFilterChip(
                  label: 'Sincronizados',
                  selected: _syncFilter == _SyncFilter.synced,
                  onTap: () => setState(() => _syncFilter = _SyncFilter.synced),
                ),
                _SyncFilterChip(
                  label: 'Con error',
                  selected: _syncFilter == _SyncFilter.error,
                  onTap: () => setState(() => _syncFilter = _SyncFilter.error),
                ),
              ],
            ),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        onTap: canManage
            ? () => context.push(AppRoutes.productManage, extra: product)
            : null,
        child: Container(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        _MiniChip(
                          label:
                              'Costo ${FormatHelpers.currency(product.cost)}',
                          color: AppColors.textSecondary,
                        ),
                        if (product.cost > 0) ...[
                          _MiniChip(
                        ...
                        if (product.origin == 'app_reporte')
                          _MiniChip(
                            label: product.synced
                                ? 'Sincronizado'
...
class _ProductCard extends StatelessWidget {
  final ProductModel product;
  final bool canManage;
  const _ProductCard({required this.product, required this.canManage});
    ...
    return InkWell(
      ...
      onTap: canManage
          ? () => Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => ProductManageScreen(product: product),
              ),
            )
          : null,
      child: Container(
...
List<ProductModel> _filterProductsForScope(
  ...
  final activeProducts = products
      .where((product) => product.isActive || product.origin == 'app_reporte')
      .toList();
...
enum _SyncFilter { all, createdFromApp, pending, synced, error }
