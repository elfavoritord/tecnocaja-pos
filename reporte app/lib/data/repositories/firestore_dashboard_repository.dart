...
class FirestoreDashboardRepository {
  ...
  Future<ReportsDashboardData> getDashboard({
    ...
    // Consulta base por fecha con límite para evitar descargar miles de docs.
    // Firestore no permite múltiples desigualdades en campos distintos, así que
    // los filtros de sucursal/caja/cajero se aplican en memoria después.
    final snapshot = await _db
        ...
        .where('createdAt', isLessThanOrEqualTo: Timestamp.fromDate(range.to))
    ...
    // ── KPIs ─────────────────────────────────────────────────────────────────
    final totalRevenue = completed.fold<double>(0, (acc, s) => acc + s.total);
    final totalTax = completed.fold<double>(0, (acc, s) => acc + s.tax);
    final grossProfit = completed.fold<double>(0, (acc, s) => acc + s.profit);
    final totalInvoices = completed.length;
    final avgTicket = totalInvoices > 0 ? totalRevenue / totalInvoices : 0.0;
    ...
    for (final s in completed) {
      final key =
          s.paymentMethod.name; // 'cash','card','credit','transfer','mixed'
    ...
    final paymentMethods = pmTotal.entries.map((e) {
      return ReportsPaymentMethod(
        ...
        total: e.value,
        percentage: totalRevenue > 0 ? (e.value / totalRevenue) * 100.0 : 0.0,
      );
    }).toList();
    ...
    for (final s in completed) {
      ...
      final prev = trendMap[key] ?? (invoices: 0, total: 0.0, tax: 0.0);
      trendMap[key] = (
