...
class FirestoreReportsRepository {
  ...
  Future<List<SaleModel>> getSales({
    ...
    if (cashRegisterId != null && cashRegisterId.isNotEmpty) {
      sales =
          sales.where((s) => s.cashRegisterId == cashRegisterId).toList();
    }
  ...
  Future<SalesSummaryResult> getSalesSummary({
    ...
    final completed =
        all.where((s) => s.status != SaleStatus.cancelled).toList();
    final cancelled =
        all.where((s) => s.status == SaleStatus.cancelled).toList();
    ...
    final avgTicket =
        completed.isNotEmpty ? totalGross / completed.length : 0.0;
    ...
    for (final s in completed) {
      final label =
          s.branchName.isNotEmpty ? s.branchName : s.branchId;
