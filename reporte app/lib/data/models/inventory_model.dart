...
class ProductModel {
  ...
  factory ProductModel.fromFirestore(DocumentSnapshot doc) {
    ...
    return ProductModel(
      ...
      sku: d['sku']?.toString() ?? d['codigo']?.toString(),
      brand: (d['brand'] ?? d['marca'] ?? '').toString(),
      unit: (d['unit'] ?? d['unidad'] ?? 'unidad').toString(),
      appliesTax: _toBool(d['appliesTax'] ?? d['aplicaItbis']),
      imageUrl: d['imageUrl'],
      branchId: d['branchId']?.toString(),
      syncStatus: (d['syncStatus'] ?? d['estadoSync'] ?? 'synced').toString(),
      synced: _toBool(
        d['sincronizado'] ?? d['synced'],
        fallbackFromStatus: 'true',
      ),
      syncError: d['syncError']?.toString(),
      origin: (d['origen'] ?? d['origin'] ?? 'pos').toString(),
      createdAt: _toDate(d['createdAt'] ?? d['fechaCreacion']),
    );
  }
